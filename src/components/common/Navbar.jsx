    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-20 px-6 py-4 bg-white shadow-md">
        <div className="flex flex-row items-center w-[98%] xsm:w-[95%] xs:w-11/12 sm:w-10/12 lg:w-11/12 xl:w-10/12 mx-auto lg:justify-between">
          ...
          <div className='flex items-center ml-4 lg:flex-col gap-y-1 xsm:ml-8 xs:ml-12 sm:ml-24 md:ml-32 lg:ml-0'>
            <div className="flex flex-col items-center p-2 lg:flex-row gap-y-2 lg:gap-4">
              <button className="flex items-center justify-center px-2 py-2 text-sm text-white transition-all duration-200 rounded-lg md:px-4 md:text-base hover:bg-light-green-200 bg-dark-green-100 hover:text-black">
                ...
                {navbarData.socialLinks.map((social, index) => {
                  ...
                  return (
                    <a key={index} href={social.link} target='_blank' rel="noopener noreferrer"
                      style={{ color: social.color, width: iconSize.width, height: iconSize.height }}
                      className="flex justify-center items-center gap-4 p-[2px] text-lg transition-all xs:border border-green-600 hover:border-none hover:bg-light-green-200 duration-300 rounded-lg cursor-pointer hover:scale-110">
            ...
            <div className="items-center hidden gap-x-8 lg:flex text-[16px] font-semibold">
              {navbarData.navItems.map((item, idx) => (
                ...
                >
                  <Link to={item.path} className="flex items-center text-gray-800 transition-all duration-200 hover:text-dark-green-200">
                    {item.label}
                  ...
                  {item.dropdown && (
                    <AnimatePresence>
                      {openDropdownIndex === idx && (
                        <motion.ul
                          className="absolute left-0 z-20 w-52 text-gray-800 rounded-lg shadow-md top-7 bg-[#cdffcd] overflow-hidden"
                        ...
                        >
                          {item.dropdown.map((dropdownItem, dropdownIdx) => (
                            <li key={dropdownIdx}
                              className={`transition-all duration-300 hover:bg-dark-green-100 hover:text-white ${dropdownIdx !== item.dropdown.length - 1 ? "border-b border-richblack-100" : ""}`}
                            >
                              <Link to={dropdownItem.path} className="block px-4 py-2">
                                {dropdownItem.label}
                              </Link>
                            </li>
                          ))}
              ...
              <div className="items-center hidden transition-all duration-300 lg:flex hover:scale-110">
                <a href="/search-product">
                  <button className="transition duration-300 text-dark-green-200 focus:outline-none hover:text-green-600">
              ...
              {navbarData.navItems.map((item, idx) => (
                <li key={idx} className="relative">
                  ...
                  >
                    ...
                    {item.dropdown && (
                      <svg className={`w-4 h-4 transform transition-transform ${openDropdownIndex === idx ? 'rotate-180' : ''}`}
                  ...
                  <AnimatePresence>
                    {openDropdownIndex === idx && (
                      ...
                      >
                        {item.dropdown.map((dropdownItem, dropdownIdx) => (
                          <li key={dropdownIdx} className={dropdownIdx !== item.dropdown.length - 1 ? "border-b border-richblack-200" : ""}>
                            <Link
                              to={dropdownItem.path}
                              className="block px-4 py-2 pl-8 transition-all duration-200 hover:bg-dark-green-200 hover:text-white"
